<script>
    ...
    onMount(()=>
    {
        ...
        pan = scale(currentPan, [0, 127], [0, track.width-(thumb.width)])
    })
    let panThumb
    let pantrack
    let voltrack
    ...
    vol = scale(y, [tr.top, tr.bottom-(rect.height/2)], [0, tr.height-(rect.height)])
    let value = ~~scale(vol, [0, tr.height-(rect.height)], [127, 0])
    ...
    pan = scale(x, [tr.left-(rect.width/2), tr.right-(rect.width/2)], [0, tr.width-(rect.width)])
    let value = ~~scale(pan, [0, tr.width-(rect.width)], [0, 127])
